...
def main_calc(solution):
    ...
    brightness = dna[:len(dna)//2]
    brightness = brightness/(len(df.index))
    ...
    # Iterate through LEDs in dna. Simulate their wavelength and power output using a gaussian distribution and distance from the 'measuring device'.
    for x in range(len(dna)):
        # Select single DNA and corresponding brightness of LED        
        led = dna[x]
        bright = brightness[x]
        # Retrieve properties of the LED from the database
        power = df['Output Power (mw)'][led] * bright
        theta = df['Viewing Angle (deg)'][led]
        wavelength = df['Peak wavelength (nm)'][led]
        FWHM = df['FWHM (nm)'][led]
