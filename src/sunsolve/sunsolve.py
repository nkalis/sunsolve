import numpy as np
import pandas as pd
from sunsolve.fitness_function import main_calc, fitness_function_factory
...
class sunsolver:
    ...
    def random_selection(self, n_leds=24, plot=True, sim_info=True):
        ...
        fits, fit, fit_weight, bins, bin_perc, xs, ys = main_calc(solution, led_df=self.leds, am=self.am0)
    ...
    def solve(self, num_generations=100,):
        """_summary_
        ...
        with tqdm(total=num_generations) as pbar:
            ...
            self.solution, self.solution_fitness, self.solution_idx = self.ga_solution.best_solution()
    ...
    def plot_solution(self):
        ...
        """
        if self.solve_ran:
            fits, fit, fit_weight, bins, bin_perc, xs, ys = main_calc(self.solution, led_df=self.leds, am=self.am0)
            plot_spectrum(fits, fit, fit_weight, bins, bin_perc, xs, ys)
    ...
    def return_solution(self):
        ...
        for leds in range(len(dna)):
            ...
            t.add_row([led, self.leds['Name'][led], self.leds['Peak wavelength (nm)'][led], round(bright,2)])
